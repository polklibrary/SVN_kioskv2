...
window.requestAnimationFrame = (function(){
    return window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function (callback){
        FroggerThread = window.setTimeout(callback, 1000 / 60);
    };
})();
...
(function(Frogger) {
    ...
    var _score = 0,
        ...
        _highScore = window.parent.FroggerHighScore,
...
(function(Frogger) {
    ...
    function renderScore() {
        ...
        Frogger.drawingSurface.fillText("1-UP", _gameBoard.columns[3], _gameBoard.grid.height / 2 + 20);
        ...
        Frogger.drawingSurface.fillText(_score, _gameBoard.columns[3], _gameBoard.grid.height + 27);
        ...
        Frogger.drawingSurface.fillStyle = "#FFF";
        Frogger.drawingSurface.fillText("HI-SCORE", _gameBoard.columns[8], _gameBoard.grid.height / 2 + 20);
        ...
        Frogger.drawingSurface.fillStyle = "#F00";
        Frogger.drawingSurface.fillText(_highScore, _gameBoard.columns[8], _gameBoard.grid.height + 27);
    }
    ...
    Frogger.observer.subscribe("high-score-change", function(newHighScore) {
        ...
        window.parent.FroggerHighScore = _score;
    });
...
Frogger.Character = (function(Frogger) {
    ...
    // Frogger.canvas.addEventListener("touchstart", function(event) {
        ...
        // }
    // }, false);
    // Frogger.canvas.addEventListener("click", function(event) {

        // // Get a reference to the position of the touch on the screen in pixels from the
        // // top-left position of the touched element, in this case the game board
        // var touchLeft = event.clientX,
    ...
    $(document).ready(function(){
        $('.frogger .gamepad .gp-up').click(function(){
            move(Frogger.direction.UP);
        });
        $('.frogger .gamepad .gp-down').click(function(){
            move(Frogger.direction.DOWN);
        });
        $('.frogger .gamepad .gp-left').click(function(){
            move(Frogger.direction.LEFT);
        });
        $('.frogger .gamepad .gp-right').click(function(){
            move(Frogger.direction.RIGHT);
        });
    });
